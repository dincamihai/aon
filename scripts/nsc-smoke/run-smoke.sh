...
#   4. nsc revocations add-user + nsc push ($SYS.REQ.CLAIMS.UPDATE).
#   5. Pub from SAME .creds → must reject.
#   6. nsc revocations delete-user + nsc push.
...
run_phase_revoke() {
  ...
  phase "PHASE D: revoke takes effect (via 'aon revoke')  team=$team  role=$role"

  CURRENT_WORK="$(mktemp -d "$WORK_ROOT/${team}.revoke.XXXXXX")"
  CURRENT_KEEP=0
  local W="$CURRENT_WORK"
  mkdir -p "$W/data" "$W/jwt" "$W/creds" "$W/team-dir"

  # Align NSC store path with what `aon revoke` expects when run with
  # HOME=$W, so the same store is visible to both raw nsc (this script)
  # and bin/aon (cmd_revoke). $AON_NSC_HOME defaults to $HOME/.aon/nsc;
  # _aon_nsc_env exports XDG_DATA_HOME=$AON_NSC_HOME/data, etc.
  export AON_NSC_HOME="$W/.aon/nsc"
  export XDG_DATA_HOME="$AON_NSC_HOME/data"
  export XDG_CONFIG_HOME="$AON_NSC_HOME/config"
  mkdir -p "$XDG_DATA_HOME" "$XDG_CONFIG_HOME"
...
EOF
  ...
  note "step 5/7: aon revoke (must fold nsc push internally)"
  # Drive the production cmd_revoke path by writing a minimal aon.toml
  # under $W/team-dir and pointing aon at it via HOME=$W (so
  # _aon_team_creds_dir + AON_NSC_HOME both resolve under our tempdir).
  cat > "$W/team-dir/aon.toml" <<EOF
[engine]
version = "0.1"
[team]
name = "$team"
...
EOF
  ...
  if [[ $revoke_rc -ne 0 ]]; then
    fail "aon revoke $role failed (rc=$revoke_rc)"
    sed 's/^/    /' "$W/aon-revoke.log" >&2
    TOTAL_FAIL=$((TOTAL_FAIL+1)); CURRENT_KEEP=1
    return 1
  fi
  ...
  if ! nsc describe account --name "$team" --field nats.revocations 2>/dev/null \
       | grep -q '"'; then
    fail "account JWT has no revocations map after aon revoke"
    TOTAL_FAIL=$((TOTAL_FAIL+1)); CURRENT_KEEP=1
  fi
  ...
  if [[ $clear_rc -ne 0 ]]; then
    fail "aon revoke clear $role failed (rc=$clear_rc)"
    sed 's/^/    /' "$W/aon-revoke-clear.log" >&2
    TOTAL_FAIL=$((TOTAL_FAIL+1)); CURRENT_KEEP=1
    return 1
  fi
