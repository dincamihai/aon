...
_aon_nsc_ensure_user() {
  ...
  case "$kind" in
    ...
    anon)
      ...
      nsc add user --account "$team" "$name" \
        --allow-pub "${_wr_kv}.request.>,\$JS.API.STREAM.MSG.GET.${_wr_str},\$JS.API.STREAM.MSG.DELETE.${_wr_str},\$JS.API.CONSUMER.CREATE.${_wr_str}.>,\$JS.API.CONSUMER.MSG.NEXT.${_wr_str}.>,\$JS.API.CONSUMER.DELETE.${_wr_str}.>" \
